...
const socialLinks = [
  { label: "WhatsApp", href: `https://wa.me/${CONTACT.whatsapp}`, icon: <WhatsAppIcon size={16} />, hover: "hover:text-green-500 hover:border-green-400/40" },
  { label: "Telegram", href: `https://t.me/${CONTACT.telegram}`, icon: <TelegramIcon size={16} />, hover: "hover:text-sky-400 hover:border-sky-400/40" },
  { label: "Instagram", href: "https://instagram.com/saherpharma", icon: <InstagramIcon size={16} />, hover: "hover:text-pink-500 hover:border-pink-400/40" },
  {
    label: "Email",
...
const paymentMethods = [
  ...
  { name: "Bank Transfer", symbol: "Bank Transfer", color: "text-slate-300",  bg: "bg-white/5 border-white/10" },
];
...
function FlaskIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M9 3h6M9 3v7l-4.5 9A1 1 0 0 0 5.4 21h13.2a1 1 0 0 0 .9-1.5L15 10V3"/>
      <path d="M6.5 14h11"/>
    </svg>
  );
}
...
export function Footer({ onEnquire }: { onEnquire?: (items: CartItem[]) => void }) {
  ...
  const whyCards = [
    {
      icon: <GlobeIcon />,
      title: "Global Export",
      desc: "Shipping to 25+ countries — Asia, Africa, Middle East, LATAM & CIS — with full documentation.",
      color: "text-sky-600 dark:text-sky-400",
      iconBg: "bg-sky-100 dark:bg-sky-900/30",
    },
    {
      icon: <FlaskIcon />,
      title: "Genuine Molecules",
      desc: "100% authentic branded formulations sourced from licensed Indian manufacturers.",
    ...
    },
    {
      ...
      desc: "COA, MSDS, and product dossiers provided with every shipment for smooth customs clearance.",
    ...
    },
    {
      icon: <ZapIcon />,
      title: "Fast Dispatch",
      desc: "Orders processed within 48 hours of confirmation. Export desk replies within 2 hours.",
  ...
  return (
    <footer className="relative">
      ...
      {/* Why SaherPharma section */}
      <div id="why" className="border-t border-b border-[var(--border)] bg-[var(--bg-2)] px-4 py-20">
        <div className="mx-auto max-w-6xl">
          <div className="mb-12 text-center">
            ...
            <h2 className="mt-4 font-display text-[2rem] font-bold text-[var(--text)] sm:text-[2.4rem]">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {whyCards.map((f) => (
              <div
                key={f.title}
                className="relative overflow-hidden rounded-2xl bg-white dark:bg-[var(--surface)] border border-[var(--border)] p-6 card-hover shadow-sm"
              >
                <div className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl ${f.color} ${f.iconBg}`}>
                  {f.icon}
                </div>
                <h3 className="font-display text-[13px] font-bold text-[var(--text)]">{f.title}</h3>
                <p className="mt-2 text-[12px] leading-relaxed text-muted">{f.desc}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 grid grid-cols-3 divide-x divide-[var(--border)] overflow-hidden rounded-2xl border border-[var(--border)] bg-white dark:bg-[var(--surface)]">
            {[
              ...
              <div key={s.label} className="flex flex-col items-center justify-center py-7 px-4 text-center">
                ...
                <div className="mt-1 text-[11px] font-medium uppercase tracking-wider text-muted">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#0f172a] dark:bg-[#070c18]">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {paymentMethods.map((m) => (
                <div key={m.name} className={`flex items-center gap-2 rounded-full border ${m.bg} px-5 py-2`}>
                  <span className={`text-[13px] font-semibold ${m.color}`}>{m.symbol}</span>
                </div>
              ))}
            </div>
        ...
        <div className="px-4 py-14">
          <div className="mx-auto max-w-6xl grid gap-12 md:grid-cols-[1.8fr_1fr_1fr_1.5fr]">
            ...
            {/* Company */}
            <div>
              <h4 className="mb-5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Company</h4>
                ...
                {companyLinks.map(([label, id]) => (
                  <li key={label}>
                    <button
                      onClick={() => scrollTo(id)}
                      onMouseEnter={hoverSound}
                      className="text-[13px] text-slate-400 transition hover:text-teal-400"
                    >
                      {label}
                    </button>
                  </li>
            ...
            {/* Catalogue */}
            <div>
              <h4 className="mb-5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Catalogue</h4>
              <ul className="space-y-3">
            ...
            {/* Contact */}
            <div>
              <h4 className="mb-5 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Contact</h4>
              <ul className="space-y-3">
                <li>
                  <a
                    href={`mailto:${CONTACT.email}`}
                    className="flex items-start gap-2.5 text-slate-400 transition hover:text-teal-400 break-all"
                  >
                ...
                <li>
                  <a href={`https://wa.me/${CONTACT.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-slate-400 transition hover:text-green-400">
                    <WhatsAppIcon size={13} />
                    <span className="text-[12px]">WhatsApp</span>
                  </a>
                </li>
                <li>
                  <a href={`https://t.me/${CONTACT.telegram}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-slate-400 transition hover:text-sky-400">
                    <TelegramIcon size={13} />
                    <span className="text-[12px]">@{CONTACT.telegram}</span>
                  </a>
                </li>
                <li>
                  <a href="https://instagram.com/saherpharma" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2.5 text-slate-400 transition hover:text-pink-400">
                    <InstagramIcon size={13} />
                    <span className="text-[12px]">@saherpharma</span>
                  </a>
                </li>
                <li className="flex items-start gap-2.5 text-slate-400">
                  ...
                  <span className="text-[12px]">{CONTACT.address}</span>
                </li>
              </ul>
            </div>
          </div>
